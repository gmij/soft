...
const RequestSoftwareDialog: React.FC<RequestSoftwareDialogProps> = ({ open, onClose }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Create GitHub issue URL with pre-filled content (internationalized)
      const isEnglish = i18n.language === 'en';
      const issueTitle = isEnglish 
        ? `[Software Request] ${values.softwareName}`
        : `[软件申请] ${values.softwareName}`;
      
      const issueBody = isEnglish 
        ? `## Software Information

**Software Name**: ${values.softwareName}

${values.additionalInfo ? `**Additional Information**: ${values.additionalInfo}` : ''}
...
`;
      ...
      if (error instanceof Error && error.message === 'Popup blocked') {
        message.error(t('request.submitError') + ' - ' + (i18n.language === 'en' ? 'Please allow popups for this site' : '请允许浏览器弹出窗口'));
      } else {
